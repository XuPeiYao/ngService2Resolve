...
// 這裡是這個LIB要匯出的模組路徑
var modulePath = './src/app/service2resolve';
...
gulp.task('build', function (done) {
  runSequence(
    ...
    function () {
      ...
      console.log('編譯結束, 編譯結果於 ./dist 目錄中, 如要發行則執行 npm publish dist');
    }
  );
});
...
gulp.task('compile-sass', function () {
  gulp
    .src('src/**/*.scss')
    .pipe(gulp_sass({ outputStyle: 'compressed' }).on('error', gulp_sass.logError)) // this will prevent our future watch-task from crashing on sass-errors
    .pipe(
    gulp.dest(function (file) {
